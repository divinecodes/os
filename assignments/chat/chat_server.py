...
def create_listen_socket(host,port):
    """ Setup the sockets our server will receive connection requests
    on"""
...
def recv_msg(sock):
	""" Wait for data to arrive on socket, then parse into messages
	...
	while not msg:
		...
		data  = data + recvd
		if b'\0' in recvd:
			# our rule is that  b'\0' will always be 
			# the last character 
			msg = data.rstrip(b'\0')
...
def send_msg(sock, msg):
	""" Send a string over socket, preparing it first """
	data = prep_msg(msg)
	sock.sendall(data)

def parse_recvd_data(data):
    """Break up raw received data into messages, delimited by null 
        byte"""
